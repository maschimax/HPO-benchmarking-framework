...
def sensor_loading():
    ...
    folder = 'C:/Users/Max/Desktop/Sensor System Production/Process Chains/Datensatz 2'
    ...
    X_new = X_new[4]
    y_process_labels = pd.DataFrame(y_process_labels[4])

    return X_new, y_process_labels

def sensor_balancing(X_train, y_train):

    y_train = pd.Series(y_train[0], index=y_train.index)
...
def sensor_loading_and_preprocessing():
    ...
    y_raw = pd.DataFrame(lab_enc.fit_transform(y_raw))
